...
class DistillationLoss(nn.Module):
    """
    Knowledge distillation loss for SAM2 image encoder
    """
    def __init__(
        self,
        ...
        self.feature_weights = feature_weights or [1.0, 1.0, 1.0, 1.0]  # 不同层特征的权重
        
    def forward(self, student_features, teacher_features, student_logits=None, teacher_logits=None):
        """
        Args:
            student_features: 学生模型的特征 [B, C, H, W]
            teacher_features: 教师模型的特征 [B, C, H, W]
            student_logits: 学生模型的输出logits (可选)
            teacher_logits: 教师模型的输出logits (可选)
        """
        total_loss = 0.0
        ...
        for i, (s_feat, t_feat) in enumerate(zip(student_features, teacher_features)):
            if i < len(self.feature_weights):
                ...
                # 计算特征蒸馏损失
                feat_loss = F.mse_loss(s_feat_norm, t_feat_norm)
                feature_loss += weight * feat_loss
                
        total_loss += feature_loss
        
        # 2. 输出logits蒸馏损失 (如果有的话)
...
class SAM2DistillationLoss(nn.Module):
    """
    SAM2特定的蒸馏损失，包含多个组件的蒸馏
    """
    def __init__(
        self,
    ...
    def forward(self, student_outputs, teacher_outputs, original_loss):
        ...
        """
        # 提取特征进行蒸馏
        student_features = student_outputs.get("backbone_fpn", [])
        teacher_features = teacher_outputs.get("backbone_fpn", [])
